...
module.exports = function(config) {
  ...
  router.get('/oauth2callback', function(req, res) {
    ...
    var createName = function(gProfile) {
      var username = gProfile.name['givenName']+ ' '+gProfile.name['familyName'].slice(0,1);
      if (!username) {
        username = gProfile.displayName;
      }
      ...
      getUserProfile(client, function(err, profile) {
        ...
        User.findOne({_id: profile.id}, function(err, user) {
          if (user) {
            ...
            var username = createName(profile);
            var user = new User({
              _id: profile.id,
